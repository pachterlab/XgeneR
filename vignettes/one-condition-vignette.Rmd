...
Run edgeR to produce raw p-values and Benjamini-Hochberg false discovery rates (corrected by the number of genes per test) for the null hypotheses of no cis regulation, stored as "beta_cis", and the null of no trans regulation ("beta_trans"). You could also run with covariates in covariate_col (they must be columns in metadata and in vector string format). 

```{r}
# run edgeR tests
fit_obj <- fit_edgeR(fit_obj)
head(fit_obj@raw_pvals[["beta_cis"]])
head(fit_obj@raw_pvals[["beta_trans"]])
head(fit_obj@BH_FDRs[["beta_cis"]])
head(fit_obj@BH_FDRs[["beta_trans"]])
```
